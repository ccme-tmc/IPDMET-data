...
title = 'Dy'
ncas, nelec, mo = myavas.avas(mf, ['Dy 4f'], minao=mol._basis['Dy'], threshold=0.5, openshell_option=2)
...
# NEVPT2
# 从已有文件 title+'_nevpt2.txt' 读取 ecorr，而不是重新计算
ecorr = np.loadtxt(title + '_nevpt2.txt')
# 防止读到单个数时变成标量
ecorr = np.asarray(ecorr)
# 加 NEVPT2 修正

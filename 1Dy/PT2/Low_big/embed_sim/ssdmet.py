...
def compute_entropy(occ_env, threshold=1e-12):
    # 计算矩阵 D 的特征值和特征向量
    eigs = occ_env

    # 选择 (0, 2) 区间内的特征值，并去除小于 threshold 的特征值
    eigs = eigs[(eigs >= threshold) & (eigs <= 2 - threshold)]
    two_eigs = 2 - eigs
    lamda = eigs * 0.5
    one_lamda = two_eigs * 0.5

    S1 = -np.sum(lamda * np.log(lamda))
    S2 = -np.sum(one_lamda * np.log(one_lamda))
    S = S1 + S2
    # 计算 S_E = 2 * S
    S_E = 2 * S
...
def build_embeded_subspace(ldm, imp_idx, lo_meth='lowdin', thres=1e-12, es_natorb=True):
    ...
    #Calculating Entropy of entanglement
    S_E = compute_entropy(occ_env, threshold=1e-12)
    print(f"Entropy of entanglement S_E: {S_E}")
...
class SSDMET(lib.StreamObject):
    ...
    def lowdin_orth(self, restore_imp = False, iaopao = False, sslpl = False):
        # lowdin orthonormalize
        ...
        if sslpl:
            OA = ['Ce.*','Al1.*','Al2.*','Al3.*','Al4.*','Al5.*','Al6.*','Al7.*','Al8.*','Al9.*','Y1.*','Y2.*']
            S_ovlp = self.mf_or_cas.get_ovlp()
            caolo = ic_helper.ic_orthogonalization(S_ovlp, OA, self.mol)
            cloao = np.linalg.inv(caolo)

        ldm = reduce(lib.dot, (cloao, self.dm, cloao.conj().T))
        return ldm, caolo, cloao
        
    def build(self, restore_imp = False, iaopao = False, sslpl = False, chk_fname_load='', save_chk=True):
        self.dump_flags()
